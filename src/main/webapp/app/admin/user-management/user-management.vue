<template>
    <div>
        <h2>
            ...
            <router-link tag="button" class="btn btn-primary float-right jh-create-entity" :to="{name: 'ScUserCreate'}">
        ...
        <div class="table-responsive" v-if="users">
            <table class="table table-striped">
                ...
                <tbody v-if ="users">
                    <tr v-for="user in users" :key="user.id" :id="user.login">
                        <td><router-link tag="a" :to="{name: 'ScUserView', params: {userId: user.login}}">{{user.id}}</router-link></td>
                        <td>{{user.login}}</td>
                        ...
                        <td class="text-right">
                            <div class="btn-group">
                                <router-link :to="{name: 'ScUserView', params: {userId: user.login}}" tag="button" class="btn btn-info btn-sm details">
                                    <font-awesome-icon icon="eye"></font-awesome-icon>
                                    <span class="d-none d-md-inline" v-text="$t('entity.action.view')">View</span>
                                </router-link>
                                <router-link :to="{name: 'ScUserEdit', params: {userId: user.login}}"  tag="button" class="btn btn-primary btn-sm edit">
            ...
            <b-modal ref="removeUser" id="removeUser" title="Confirm delete operation" v-bind:title="$t('entity.delete.title')" @ok="deleteUser()">
                <div class="modal-body">
                    <p id="sc-delete-user-heading" v-text="$t('userManagement.delete.question', { 'login': removeId})">Are you sure you want to delete this user?</p>
                </div>
